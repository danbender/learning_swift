...
//  99RedBallons
//
//  Created by Dan Bender on 04/06/15.
...
class ViewController: UIViewController {
    ...
    @IBAction func previousBalloonBarButtonItemPressed(sender: UIBarButtonItem) {
        ...
        if currentIndex == 0 {
            currentIndex = Int(arc4random_uniform(UInt32(4))) + 1
        }
        

    }
    
    @IBAction func nextBalloonBarButtonItemPressed(sender: UIButton) {
        let balloon = balloons[currentIndex]
        balloonsLabel.text = "\(balloon.number) balloon"
        backgroundImageView.image = balloon.image
        currentIndex += 1
